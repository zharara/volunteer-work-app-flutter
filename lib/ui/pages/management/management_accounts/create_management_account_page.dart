...
class CreateManagementAccountPage extends StatefulWidget {
  const CreateManagementAccountPage({Key? key}) : super(key: key);

  @override
  State<CreateManagementAccountPage> createState() => _CreateManagementAccountPageState();
}

class _CreateManagementAccountPageState extends State<CreateManagementAccountPage> {
  ...
  Widget build(BuildContext context) {
    return BlocListener<CreateManagementAccountCubit,
        CreateManagementAccountState>(
      listener: (context, state) {
        if (state is CreateManagementAccountError) {
          showNetworkException(context: context, error: state.error);
        }
        if (state is CreateManagementAccountSuccess) {
          showSuccessSnackBar(
            context: context,
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('إنشاء حساب إدارة جديد'),
        ),
                ...
                ElevatedButton(
                  onPressed: () {
                    if (_formKey.currentState?.validate() ?? false) {
                      BlocProvider.of<CreateManagementAccountCubit>(context)
                          .createAccount(
                  ...
                  Container(
                    ...
                    child: const Center(child: Text('إنشاء حساب إدارة')),
                  ),

                ),
                BlocBuilder<CreateManagementAccountCubit,
                    CreateManagementAccountState>(
                  builder: (context, state) {
                    if (state is CreateManagementAccountLoading) {
